...
- Added bundled `google/gemini-3.7-flash` and `google-gemini-cli/gemini-3.7-flash` catalog entries. Gemini 3.7 Flash accepts only `low`, `medium`, and `high` thinking levels; `minimal` is rejected because the official API returns an error.
