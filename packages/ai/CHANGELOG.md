...
- OpenCode Go chat-completions requests to the canonical Go endpoint now send the provider-required `x-opencode-session` header using the coding-agent's existing opaque provider session id. The value remains stable across turns and every retry layer in one conversation, differs with the conversation lifecycle, cannot be replaced by custom request transforms, and is never added to OpenCode Zen or unrelated OpenAI-compatible endpoints (#5295).
