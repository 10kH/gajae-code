...
- OpenCode Go chat-completions requests now send the provider-required `x-opencode-session` header using the coding-agent's existing opaque provider session id. The value remains stable across turns and retries in one conversation, differs with the conversation lifecycle, and is never added to OpenCode Zen or unrelated OpenAI-compatible endpoints (#5295).
