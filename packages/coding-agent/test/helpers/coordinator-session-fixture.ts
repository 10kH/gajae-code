...
export type CoordinatorSessionFixtureOptions = {
	...
	/**
	 * Stamp the canonical WAL session as idle this many milliseconds ago. The
	 * idle reaper reads the rebuilt projection, whose timestamps come from the
	 * WAL, so staleness must live there. Defaults to idle-eligible.
	 */
	idleAgeMs?: number;
	/**
...
export async function writeDurableCoordinatorSession(
	...
	const staleAt = new Date(Date.now() - (options.idleAgeMs ?? FIXTURE_IDLE_AGE_MS)).toISOString();
	...
	const intent: CanonicalCreateIntentV1 = {
		...
		initial_events: [
			{ kind: "session.registered", entity: "session", entity_id: options.sessionId, created_at: staleAt },
		],
	};
	await createSessionTransaction(paths, intent);
	if (options.idleAgeMs !== 0 || options.activeTurn) {
		...
		// canonical idle/capacity fixtures).
		await withSessionTransaction(paths, options.sessionId, async (current: CoordinatorSessionTransactionV1) => {
			const nextRevision = current.revision + 1;
			current.canonical.session.created_at = staleAt;
			current.canonical.session.updated_at = staleAt;
			if (options.activeTurn) {
				current.canonical.turns[options.activeTurn.turnId] = {
					...
					prompt: { text: "fixture active turn", created_at: staleAt, source: "coordinator" },
