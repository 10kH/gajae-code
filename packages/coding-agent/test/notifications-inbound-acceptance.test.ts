...
describe("notification inbound acceptance and reaction correction", () => {
	test("fenced predecessor and notification-origin policy suspension are explicit drops", () => {
		expect(
			notificationInboundAdmission({ inboundFenced: true, policySuspended: false, notificationOrigin: true }),
		).toEqual({ outcome: "drop", reason: "inbound_fenced" });
		expect(
			notificationInboundAdmission({ inboundFenced: false, policySuspended: true, notificationOrigin: true }),
	...
	test("non-notification clients are not swallowed by provisional notification policy", () => {
		expect(
			notificationInboundAdmission({ inboundFenced: false, policySuspended: true, notificationOrigin: false }),
