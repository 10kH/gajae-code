 ...
 * entry points.
 *
 * Whitespace is the same hazard, not a separate one: `path.join("   ",
 * "notifications")` yields `"   /notifications"`, equally relative to the cwd.
 * The value is trimmed first so a blank-but-present flag is absent too.
 */
function argValue(argv: string[], name: string): string | undefined {
	const i = argv.indexOf(name);
	const value = (i >= 0 ? argv[i + 1] : undefined)?.trim();
...
export async function runDaemonSmoke(opts: { agentDir?: string } = {}): Promise<void> {
	// `??` would accept an empty (or blank) dir and skip the temp dir entirely,
	// writing the smoke artifacts into the current working directory.
	const agentDir = opts.agentDir?.trim() || fs.mkdtempSync(path.join(process.cwd(), ".telegram-daemon-smoke-"));
