...
- **Complete** — after a structured verdict receipt is persisted under `.gjc/_session-{sessionid}/autoresearch/`, call `goal({"op":"complete"})` exactly once for the matching nonterminal mission goal. Any verdict completes the goal-tracking pass, including best-effort and inconclusive verdicts; an inconclusive mission stays open for follow-up, but its current goal pass is complete. If completion or the following clear fails, retain the durable state and retry on re-entry rather than issuing a second completion or dropping a completed goal.
