...
Autoresearch drives its mission through GJC goal mode the same way `ultragoal` does: an inline goal keeps the autonomous continuation loop nudging the agent through intake → research → verdict until the mission's structured verdict receipt exists. The integration is instruction-only — the agent calls the unified `goal` tool itself; `gjc autoresearch` commands and hooks never mutate goal state, and no runtime guard, nudge budget, or `ask` blocking applies to research missions.

Lifecycle:

- **Create** — right after the mission artifact is written (both spec and cold intake), call `goal({"op":"get"})`; when no goal is active, call `goal({"op":"create","objective":"Drive the autoresearch mission '<mission objective>' to a persisted structured verdict receipt"})`. No goal exists during intake questioning, so `ask`-driven cold-intake clarification is unaffected.
- **Collision rule** — never call `goal({"op":"create"})` while a different inline goal is active; run the mission without goal mode instead.
- **Resume rule** — when resuming a mission (`gjc autoresearch read --json`) with no other inline goal active, re-create the goal under the same collision rule when the mission has no persisted verdict receipt OR its latest verdict is inconclusive (the mission's retained follow-up state); the re-created goal tracks the next verdict pass. Never re-create over a conclusive verdict receipt.
- **Complete** — call `goal({"op":"complete"})` exactly when the structured verdict receipt is persisted under `.gjc/_session-{sessionid}/autoresearch/`. Any verdict status completes the goal, including best-effort and inconclusive verdicts: an inconclusive mission stays open for follow-up, but the goal tracking this verdict pass is complete.
- **Auto-clear** — for every non-inconclusive verdict, retire mission state in the same completion step: run `gjc autoresearch clear` immediately after `goal({"op":"complete"})` so no stale verdict-complete mission artifact lingers. This automatic clear follows a completed goal and never calls `goal({"op":"drop"})`. Inconclusive verdicts skip the clear — the mission stays open for follow-up under the existing non-terminal contract.
- **Drop** — call `goal({"op":"drop"})` only while the mission's inline goal is still active: on a manual `gjc autoresearch clear` (pre-verdict or open-inconclusive mission), on handoff to `/skill:ralplan`, `/skill:deep-interview`, or `/skill:ultragoal`, and on user cancel. The post-verdict auto-clear is exempt — its goal is already complete, and a completed goal is never dropped.
...
The mission ends on one mission-level structured verdict: `status` (structured data, not a pinned enum), `evidence[]`, `caveats[]`, and the `evaluator` identity. The verdict is self-issued by default. An optional critic pass records a separate evaluator identity on the verdict (`critic_receipt`), distinct from the mission agent's. The verdict is best-effort, not a rigid per-lane checklist: missing lanes surface as caveats, not automatic failure. An inconclusive verdict is explicitly non-terminal — the mission stays open for follow-up rather than being closed as finished. Every other verdict is terminal for the mission artifact as well: issuing it ends with `gjc autoresearch clear` in the same step (see [Goal mode (nudge until verdict)](#goal-mode-nudge-until-verdict)).
