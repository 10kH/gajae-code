...
- `ctrl+backspace` now deletes the previous word in the composer. `tui.editor.deleteWordBackward` has always declared `ctrl+w`, `alt+backspace` **and** `ctrl+backspace`, but the editor's literal chain only implemented the first two, so the third declared chord silently did nothing. Raw `0x08` still means plain backspace on hosts that are not Windows Terminal, which is where that byte is ambiguous.
