...
## [Unreleased]
- Display-safe escaped tool calls now require bounded raw escape-position/scalar evidence before the post-resample U+2014 exemption can execute. Printable ASCII escapes are retained too, so one-nibble mutations such as `\u00b7` → `\u0077` and `\u2026` → `\u0026` fail closed instead of disappearing after JSON decoding. Evidence contains only offsets, scalars, location kinds, unambiguous hashed array-index-free paths, a bounded total, and a process-local integrity tag; overflow, malformed or duplicate-key JSON, missing/altered evidence, escaped keys, dotted-key ambiguity, excessive nesting, and every scalar except exact U+2014 reject terminally. Existing resample/terminal debug records remain shape-only and never include argument payloads.
