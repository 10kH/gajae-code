...
## [Unreleased]
- Escaped-non-ASCII turn discards and terminal per-call rejections are now logged (shape-only: tool names, attempt number, whether steering already rode the attempt, managed vs in-loop). The discarded turn never reaches durable history, so the defect previously left no trace anywhere except the surfaced tool error, and its fire rate could only be recovered by scraping session transcripts.
