...
2. `open` resolves browser kind with `resolveBrowserKind()`:
   ...
   - `app.browser: "chrome"` → `{ kind: "chrome-profile" }`. `path` defaults to the installed Chrome/Chromium (`resolveSystemChromium()`), `profile_directory` to `"Default"`, and `user_data_dir` to the per-OS Chrome user data directory containing that profile (`discoverDefaultChromeProfile()`); supplied `path`/`user_data_dir` are resolved against the session cwd. `background`, `no_focus`, and optional `cdp_port` are copied through. Errors only when no Chrome/Chromium binary exists or the requested profile is not found under the default roots.
