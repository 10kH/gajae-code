...
- Provider-agnostic open-model profiles are named by the model families they require. Single-family choices are `open-weights-{glm,deepseek,kimi,luna,spark}`; two-family choices are `open-weights-glm-deepseek`, `open-weights-kimi-deepseek`, `open-weights-kimi-glm`, `open-weights-spark-deepseek`, and `open-weights-spark-luna`; `open-weights-kimi-glm-deepseek` uses all three open-weight families; `open-weights-all` adds GPT-5.6 Luna. The Muse Spark 1.2 profiles keep `muse-spark-1.2` on the default (`medium`), planner (`high`), critic (`high`), and architect (`xhigh`) roles and differ only in the executor: `open-weights-spark` stays on Spark (`low`), `open-weights-spark-deepseek` uses `deepseek-v4-flash:high`, and `open-weights-spark-luna` uses `gpt-5.6-luna:high`. Choose the smallest combination covered by the models available through your configured providers. Every selector is a bare final-segment alias with `required_providers: []`, so each family may come from any authenticated bundled or custom provider under Provider Priority. GPT-5.6 Luna is proprietary despite its inclusion in this group.
...
- `grok-build-pro` — Grok Build OAuth preset (`/login grok-build`) that keeps `grok-build/grok-composer-2.5-fast` on the default, planner, and critic roles and `grok-build/grok-build` on the executor and architect roles; it is the recommended preset after a successful Grok Build login.
