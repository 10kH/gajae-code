...
// keep both: dev GLM-5.3 paragraph, then contributor Gemini 3.7 Flash paragraph
...
Gemini 3.7 Flash (`google/gemini-3.7-flash`, `google-gemini-cli/gemini-3.7-flash`) is selectable from the bundled catalog. It uses `google-level` thinking and accepts only `low`, `medium`, and `high`; `minimal` is rejected because the official Gemini API returns an error. Provider defaults stay on the existing Pro-class models.
