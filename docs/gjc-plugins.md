...
- **MCP policy** (install + runtime connect): HTTPS-only for `http`/`sse`; private/loopback/link-local/unique-local/multicast and the `169.254.169.254` metadata endpoint are denied across IPv4, IPv6, IPv4-mapped/compatible, zone-id and trailing-dot forms; URL credentials and CRLF headers are rejected; DNS is re-resolved before connect (rebinding defence). `stdio` servers are confined to the plugin root. Bare `node`/`bun` launchers require the bundled entrypoint as the first argument (no launcher options before it); the compiler hashes every effective-`cwd` file selected by the shared launcher classifier and runtime rewrites the entrypoint to its installed absolute path. Root-confined executable commands are likewise copied and made executable. Unexpected files added to the installed tree quarantine the bundle, and manifest/config/file hashes plus the exact launch plan are rechecked immediately before process creation. Post-entrypoint server arguments remain opaque except for no-expansion/path confinement checks. Plugin Bun processes use an empty OS config and disable ambient dotenv loading and package auto-install.
