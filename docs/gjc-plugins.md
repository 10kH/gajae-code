...
- **MCP policy** (install + runtime connect): HTTPS-only for `http`/`sse`; private/loopback/link-local/unique-local/multicast and the `169.254.169.254` metadata endpoint are denied across IPv4, IPv6, IPv4-mapped/compatible, zone-id and trailing-dot forms; URL credentials and CRLF headers are rejected; DNS is re-resolved before connect (rebinding defence). `stdio` servers are confined to the plugin root and support only bare `node`/`bun` with the bundled entrypoint as the first argument; launcher options, `--`, path-qualified commands, shebang aliases, and unsupported entrypoint formats fail closed. The compiler hashes every effective-`cwd` file selected by the shared classifier. Immediately before each spawn, runtime rebinds the canonical registry identity and complete copied-file manifest, reads every authenticated file and the selected interpreter through stable file handles, and writes both into a unique owner-only launch capsule outside workspace/plugin roots. Node comes only from a root-controlled PATH link or a manager root present in the OS-provided initial environment; later dotenv/environment mutation cannot grant interpreter authority. The child executes only that private interpreter and imports the entrypoint by file URL from the immutable snapshot, so relative imports work while bare/absolute package resolution, CommonJS `createRequire`, and native modules cannot escape authenticated files. Each launch has independent authority, including concurrent connections, and the parent removes its capsule after confirmed exit or failed spawn. Snapshot admission is bounded to 8,192 files and 128 MiB; interpreter capture is bounded to 512 MiB. POSIX is supported; bundled stdio MCPs fail closed on Windows until equivalent owner-DACL and descendant-tree lifecycle authority exists. Post-entrypoint server arguments remain literal argv except for no-expansion/path confinement checks. Plugin Bun processes use an empty OS config and disable ambient dotenv loading and package auto-install.
